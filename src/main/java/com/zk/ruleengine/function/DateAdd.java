 ...
 * 日期时间加法运算
 * <p>
 * 传入3个参数，分别是被增加的日期时间、增加的值、时间单位（天时分秒）
 *
 * @author zk
 */
public class DateAdd implements Function<Object, Object> {
    ...
    public Object execute(Evaluator evaluator, List<Object> args) {
        ...
        if (!(unit instanceof String)) {
            throw new IllegalArgumentException("[日期时间加法运算]操作的单位可选[天,时,分,秒]");
        }
        ...
        if (time instanceof LocalDate) {
            ...
            if (!"dayUnit".equals(timeUnit)) {
                throw new IllegalArgumentException("[日期时间加法运算]LocalDate类型未知的时间单位");
            }
            ...
            switch (timeUnit) {
                ...
                default:
                    throw new IllegalArgumentException("[日期时间加法运算]LocalDateTime类型未知的时间单位");
            }
            ...
            switch (timeUnit) {
                ...
                default:
                    throw new IllegalArgumentException("[日期时间加法运算]Time类型未知的时间单位");
            }
            ...
            switch (timeUnit) {
                ...
                default:
                    throw new IllegalArgumentException("[日期时间加法运算]Timestamp类型未知的时间单位");
            }
        } else {
            throw new IllegalArgumentException("[日期时间加法运算]不支持的时间类型");
        }
    }
